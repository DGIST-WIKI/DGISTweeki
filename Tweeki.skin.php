...
<?php
		...
		{
			$skin->renderSidebar( 'left', 'col-md-' . $sidebar_width );
		}

		if( !$skin->checkEmptiness( 'sidebar-right' ) )
		{
			$skin->renderSidebar( 'right', 'col-md-' . $sidebar_width . ' col-md-offset-' . $sidebar_offset );
		}
	...
	public function renderContent()
		...
		if ( $this->checkVisibility( 'firstHeading' ) )
	       	{ ?>
	...
	function renderTOC( $skin, $context )
  {
		...
		$sectionInfo = $dbr->selectRow(
			'textbook_section',
			array( 'txbsec_id', 'txbsec_textbook' ),
			array( 'txbsec_title' => $skin->getSkin()->getTitle() ),
			__METHOD__,
			array()
		);
		$toc = '';
	...
	function renderSearch( $skin, $context )
		...
		if( $context == 'navbar-right' )
          ...
          if($this->data['loggedin'] == 0){
              echo '<a href= "https://wiki.dgist.ac.kr/index.php/special:login" class="navbar-brand navbar-right"><span class="glyphicon glyphicon-user"><glyphicon glyphicon-heart></span> 로그인</a>';
          }
